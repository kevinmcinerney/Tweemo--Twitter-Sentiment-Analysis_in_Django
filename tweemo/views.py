...
def pull_tweets(q):
	...
	for word in terms:
		...
		if word == 'or' or word == 'OR':
			or_check = True
		print str(or_check) + str(' or checker.....')
		if word != 'and' and word != 'or' and word != 'AND' and word != 'OR' and word != '"':
			search_list.append(word.lower())
			...
			for tweet in searched_tweets:
				...
				for search_term in search_list:
					if search_term not in tweet_list_lower and or_check == False:
						#print str(search_term) + str(' is absent')
						query_in_text = False
						break
					elif or_check == True:
						if search_term in tweet_list_lower:
							print 'working or here mate'
							query_in_text = True
							break
						else:
							query_in_text = False
						
				print str(query_in_text) + str(' tweet ^^')
				if query_in_text:
