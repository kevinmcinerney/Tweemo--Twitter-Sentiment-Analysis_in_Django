...
def send_processed_tweet_to_db(posts,country, tweet):
	...
	m_names = set(line.strip() for line in open('/home/kevin/django-kevin/bin/b_twitter/assets/Dictionaries/male.txt','r'))
	w_names = set(line.strip() for line in open('/home/kevin/django-kevin/bin/b_twitter/assets/Dictionaries/female.txt','r'))
	if tweet.text:	
		...
		for i in range(0,len(words)):
			...
			w = words[i].lower()
			ws = squeeze(w)
			if w not in stopw and w not in w_names and w not in m_names and len(w) > 2 and w in scores:
				print w
				tot += scores[w]
				matches.append(w)
			...
			elif ws not in stopw and ws not in w_names and ws not in m_names  and len(words[i]) > 2 and ws in scores:
				tot += scores[ws] + 1
				matches.append(ws)
				consecutive_sentiment_checker.append(scores[ws])
				if consecutive_sentiment_checker[i-1] > 0:
					tot += 1
				elif consecutive_sentiment_checker[i-1] < 0:
					tot -= 1
