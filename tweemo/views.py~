...
def pull_tweets(q):
	...
	for time in time_list:
		for country in country_dictionary:
			...
			searched_tweets = tweepy_search(q,"en",time[0],time[0] + timedelta(days=1),country_dictionary[country],10)
...
def send_processed_tweet_to_db(posts,country, tweet, stopw, negation, boosterwords, scores, emoticons, eng):
	...
	if tweet.text:
		...
		for i in range(0,len(words)):
			...
			elif i < (len(words)-1) and squeeze(words[i]) == '!':
				tot = exclamation_boost(consecutive_sentiment_checker,ws_score,i,tot)
    ...
    for hashtag in hashtags:
        ...
        for x in range(0,len(hashtag)+1):
		    ...
		    if validate_hashtag_gap(hashtag[0:x].lower(),eng) == False:
                        print hashtag[0:x].lower()
                        continue
                    if validate_hashtag_gap(hashtag[y:-1].lower(),eng) == False:
                        print hashtag[0:x].lower()
                        continue
                    idx = y
