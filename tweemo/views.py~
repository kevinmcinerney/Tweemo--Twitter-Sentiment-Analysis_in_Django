...
def pull_tweets(q):
	...
	context_words_dic = {}
	for word in vocab:
	    if word not in stopw and word not in scores and word not in search_list and word not in punctuation and squeeze(word) not in punctuation and word not in 'ru#"' and len(word) > 1 and word not in eng:
